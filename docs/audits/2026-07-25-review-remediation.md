...
| runtime / legacy surface / scene 精确计数不能只写在文档 | **Fixed（v1.3.0 更新）** | `PersonaContract.g.cs` 生成 `ExpandedRuntimeRows=30000`、`ExpectedAuthoredRuntimeRows=30000`、`ExpectedLegacySurfaceRows=0`、`SemanticSceneCount=1190`；`PersonaCorpus.Build` 同时拒绝 runtime、authored 与 legacy 分布偏差。 | C# `PersonaCorpusTests`、Python contract/build/validator 测试、generator `--check` 与 30,000 行 authorship ledger 门禁。 |
